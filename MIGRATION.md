...
- **Use Run-SystemTests.ps1 for all container management:** Never manually run `docker compose up/down/restart` commands. The script manages its own container lifecycle. Use `-SkipTests` to start systems, `-Restart` to force restart, `-Suite <id>` for specific suites.
...
- **Copy first, then delete:** When porting files, always copy the source file verbatim first, then surgically remove the specific lines for tax/coupon/cancel/deliver. Never rewrite or regenerate code from description — that introduces subtle bugs (e.g. wrong assertion methods, missing attributes). This applies to:
  ...
  - **Test code (ATDD)** — verbatim copy from eshop-tests, then remove tax/coupon/cancel/deliver code, then sync to the **Java starter version** which is the reference (`system-test/java/` may have diverged from eshop-tests due to ongoing changes — always diff against the Java starter tests and align to them, not eshop-tests). Syncing includes: matching test method names, matching test content/logic, adding missing DSL methods to the .NET/TypeScript DSL layers (e.g. `WithWeekday()`, `WithWeekend()`), and removing extra test files not present in Java.
