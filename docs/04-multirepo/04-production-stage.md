...
In the step `Tag Docker Images for Production`, set the value `GITHUB_TOKEN: ${{ secrets.GHCR_TOKEN }}`.
...
2. Cross-repository final version tagging works with `GHCR_TOKEN`
