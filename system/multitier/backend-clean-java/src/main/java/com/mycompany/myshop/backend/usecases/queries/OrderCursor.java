 ...
 *
 * <p>The tiebreaker is the order number rather than the surrogate {@code id}. Textbook keyset
 * pagination reaches for the primary key, and this codebase has no primary key to reach for -- a
 * domain {@code Order} carries no {@code Long id}, the adapter resolves it on the way to the table.
 * The constraint forces the honest version: a cursor is handed to a client, so its key has to be a
 * column that is unique, stable, and already public. That is exactly what {@code order_number} is,
 * and a surrogate id is none of the three.
 *
 * <p>Both fields together, never either alone. {@code order_timestamp} is not unique, so a cursor
