package com.mycompany.myshop.backend.infrastructure.persistence.readers;

import com.mycompany.myshop.backend.usecases.queries.report.ViewSalesReportCouponEffectivenessResponse;
import com.mycompany.myshop.backend.usecases.queries.report.ViewSalesReportRevenueByCountryMonthResponse;
import com.mycompany.myshop.backend.usecases.queries.report.ports.SalesReportReader;
import com.mycompany.myshop.backend.usecases.queries.report.ViewSalesReportTopSkuResponse;
...
@Component
public class JpaSalesReportReader implements SalesReportReader {
    ...
    public JpaSalesReportReader(EntityManager entityManager) {
        this.entityManager = entityManager;
    }

    @Override
    public List<ViewSalesReportRevenueByCountryMonthResponse> revenueByCountryAndMonth() {
        ...
        return rows(query).stream()
                .map(JpaSalesReportReader::toRevenue)
                .toList();
    }

    @Override
    public List<ViewSalesReportTopSkuResponse> topSkusByRevenue(int limit) {
        var query = entityManager.createNativeQuery(TOP_SKU_SQL, Object[].class)
                .setParameter(STATUS, NOT_CANCELLED)
                .setParameter(LIMIT, limit);

        return rows(query).stream()
                .map(JpaSalesReportReader::toTopSku)
                .toList();
    }

    @Override
    public List<ViewSalesReportCouponEffectivenessResponse> couponEffectiveness() {
        var query = entityManager.createNativeQuery(COUPON_EFFECTIVENESS_SQL, Object[].class)
                .setParameter(STATUS, NOT_CANCELLED);

        return rows(query).stream()
                .map(JpaSalesReportReader::toCouponEffectiveness)
                .toList();
    }
    ...
    private static ViewSalesReportRevenueByCountryMonthResponse toRevenue(Object[] row) {
        return new ViewSalesReportRevenueByCountryMonthResponse(
                (String) row[0],
                instant(row[1]),
                count(row[2]),
                count(row[3]),
    ...
    private static ViewSalesReportTopSkuResponse toTopSku(Object[] row) {
        return new ViewSalesReportTopSkuResponse(
                (String) row[0],
                count(row[1]),
                count(row[2]),
                decimal(row[3]));
    ...
    private static ViewSalesReportCouponEffectivenessResponse toCouponEffectiveness(Object[] row) {
        return new ViewSalesReportCouponEffectivenessResponse(
                (String) row[0],
                integer(row[1]),
                integer(row[2]),
                count(row[3]),
