...
//
// The list query is native rather than JPQL, for the same reason JpaSalesReportReader is: JPQL has
...
@Component
public class JpaOrderReader implements OrderReader {
    ...
    public JpaOrderReader(EntityManager entityManager) {
    ...
    // does not have.
    @Override
    public Page<BrowseOrderHistoryItemResponse> listOrders(String orderNumberFilter, PageSpec page) {
        ...
        var items = rows(query).stream().map(JpaOrderReader::toListItem).toList();
    ...
    @Override
    public Optional<ViewOrderDetailsResponse> findOrderDetail(String orderNumber) {
        return entityManager.createQuery(DETAIL_SELECT, Object[].class)
                .setParameter(ORDER_NUMBER, orderNumber)
                .getResultList().stream()
                .findFirst()
                .map(JpaOrderReader::toDetail);
    }
    ...
    private static BrowseOrderHistoryItemResponse toListItem(Object[] row) {
        return new BrowseOrderHistoryItemResponse(
                (String) row[0],
                instant(row[1]),
                (String) row[2],
                (String) row[3],
    ...
    private static ViewOrderDetailsResponse toDetail(Object[] row) {
