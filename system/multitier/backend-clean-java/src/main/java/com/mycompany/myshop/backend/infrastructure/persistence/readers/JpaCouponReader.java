...
// See JpaOrderReader for why the read side looks like this. The one thing specific to coupons
...
public class JpaCouponReader implements CouponReader {
    ...
    // See JpaOrderReader#LIST_ORDER for what OFFSET costs. Coupons are the list where it costs least:
    ...
    public JpaCouponReader(EntityManager entityManager) {
        this.entityManager = entityManager;
    }

    @Override
    public Page<BrowseCouponsItemResponse> listCoupons(PageSpec page) {
        ...
        var items = rows(query).stream().map(JpaCouponReader::toListItem).toList();
    ...
    private static BrowseCouponsItemResponse toListItem(Object[] row) {
        return new BrowseCouponsItemResponse(
                (String) row[0],
                (BigDecimal) row[1],
                instant(row[2]),
                instant(row[3]),
    ...
    // Native results are typed by the driver rather than by a mapping. See JpaOrderReader#instant.
